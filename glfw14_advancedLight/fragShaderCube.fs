...
float ShadowCalculation(vec4 fragPosLightSpace, vec3 normal, vec3 lightDir)
{
    ...
    // Manually add bias too prevent Shadow Acen
    float bias = max(0.008 * (1.0 - dot(normal, lightDir)), 0.005);
    // float bias = 0.001; // because we used CullFace strategy, So we just need a very small (even 0) to prevent acne
...
void main()
{
    ...
    if(blinn)
    {
        ...
        spec = pow(max(dot(normal, halfwayDir), 0.0), 16.0); // and here is Normal with HalfwayDir
    }
    else
    {
