...
int main()
{
	...
	//shader.setVec3("light.specular", lightColor); // High Light's color
	...
	for (; !glfwWindowShouldClose(window);)
	{
		...
		/* Translate After Scaling will cause unexpected outcome */
		...
		// abs(sin(glfwGetTime()))
		shader.setVec3("emissionControl", glm::vec3(0.1f)); // we use abs to prevent color reverse
		...
		auto specularW{ glm::vec3(1.0) };
		auto specularR{ glm::vec3(1.0f, 0, 0) };
		auto specularG{ glm::vec3(0, 1.0f, 0) };
		auto specularB{ glm::vec3(0, 0, 1.0f) };

		/* Point Light */
		//shader.setVec3("light.position", lightPosW);
		...
		// specular is fixed here
		//shader.setFloat("light.constant", 1.0f);
		//shader.setFloat("light.linear", 0.09f);
		//shader.setFloat("light.quadratic", 0.032f);

		/* Mutiple PointLights */
		auto lightPos1{ lightPosW };
		auto lightPos2{ glm::vec3(lightPosW.x + 2.0f, lightPosW.y, lightPosW.z) };
		auto lightPos3{ glm::vec3(lightPosW.x, lightPosW.y + 2.0f, lightPosW.z) };
		auto lightPos4{ glm::vec3(lightPosW.x, lightPosW.y, lightPosW.z + 2.0f) };

		std::vector pointLightPos{ lightPos1, lightPos2, lightPos3, lightPos4 };
		std::vector lightColors{ glm::vec3(1.0f), glm::vec3(1.0f, 0, 0), glm::vec3(0, 1.0f, 0), glm::vec3(0, 0, 1.0f) };
		std::vector specularColl{ specularW, specularR, specularG, specularB };

		/* Inject Pointlights' Properties and Render the PointLights */
		shader.use();
		for (std::size_t i{ 0 }; i < NR_POINT_LIGHTS; ++i)
		{
			std::string base{ "pointLights[" + std::to_string(i) + "]" };
			shader.setVec3(base + ".position", pointLightPos[i]);
			shader.setVec3(base + ".ambient", glm::vec3(0.2f) * lightColors[i]); // ambinet is defined by lightColor
			shader.setVec3(base + ".diffuse", glm::vec3(0.5f) * lightColors[i]); // the real color of light source is too
			shader.setVec3(base + ".specular", specularColl[i]); // set the lightSource's Specular Color
		...
		auto dlColor{ glm::vec3(0.5f, 0.5f, 0.8f) };
		shader.setVec3("dl.direction", -0.2f, -1.0f, -0.3f);
		shader.setVec3("dl.ambient", glm::vec3(0.2f) * dlColor);
		shader.setVec3("dl.diffuse", glm::vec3(0.5f) * dlColor);
		...
		for (std::size_t i{ 0 }; i < NR_POINT_LIGHTS; ++i)
		{
			auto modelPoint{ glm::mat4(1.0f) };
			modelPoint = glm::translate(modelPoint, pointLightPos[i]);
			modelPoint = glm::scale(modelPoint, glm::vec3(0.3f));
			modelPoint = glm::rotate(modelPoint, glm::radians(selfAngle), glm::vec3(0.0f, 1.0f, 0.0f)); // rotate by the Y-axis(Local) Rotation
			lightShader.setMat4("model", modelPoint);
			lightShader.setMat4("view", view);
			lightShader.setMat4("projection", perspProj);
