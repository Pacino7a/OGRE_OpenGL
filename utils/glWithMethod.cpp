...
unsigned int getUBO(int bindingPoint, unsigned int shaderID, std::size_t structureSize, const char* shaderStructName)
{
	...
	glBufferData(GL_UNIFORM_BUFFER, structureSize, NULL, GL_STATIC_DRAW); // we don't have any dataSource assign to, we'll do this later
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
	// Link
	/* for shader */
	unsigned int MatricesBlockIndex{ glGetUniformBlockIndex(shaderID, shaderStructName) }; // get Block Index in the Shader(GPU)
	glUniformBlockBinding(shaderID, MatricesBlockIndex, 0); // GPUMem(shader) Links to Binding Point `0`
