...
glm::mat4 Camera::getViewMatrix() const
{
	...
	// remember the middle parameter is pos + front(real heading to)!
}
...
glm::mat4 Camera::getLookAt(const glm::vec3& position, const glm::vec3& target, const glm::vec3& up) const
{
	...
	glm::vec3 direction{ glm::normalize(position - target) }; // (pos-targ) -> (pos-(pos+front)) == (-front) == Zdir

	glm::vec3 temp_up{ up }; // we set up here
	...
	resultLeft = glm::transpose(resultLeft); // transpose
	glm::mat4 resultRight{ glm::mat4(1.0f) };
	resultRight[3][0] = -position.x; // Notice: [col][row]
